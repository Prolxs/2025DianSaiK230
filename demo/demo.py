...
def on_button_click(component, event):
    """按钮回调函数 - 测试设置值功能"""
    global slider1, slider2, slider3, double_click_text
    
    if event == 3:  # 双击事件
        print("===== 按钮被双击！ =====")
        # 更新双击状态文本
        double_click_text.set_text("状态: 检测到双击!")
        double_click_text.set_text_color((255, 100, 100))
    else:  # 单击事件
        print("===== 按钮被点击！重置滑块数值 =====")
        try:
            if slider1:
            ...
            print("滑块数值已重置")
            # 更新状态文本
            double_click_text.set_text("状态: 滑块已重置")
            double_click_text.set_text_color((100, 255, 100))
        except Exception as e:
            print(f"重置滑块时出错: {e}")
            double_click_text.set_text("状态: 重置出错!")
            double_click_text.set_text_color((255, 50, 50))
...
def create_enhanced_ui():
    ...
    # 状态显示
    double_click_text = ui.add_static_text(120, 220, 14, "状态: 就绪", (100, 255, 100))
