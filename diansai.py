...
laser_canvas = LaserCanvas(400, 240)  # 创建绘图画布
laser_canvas.set_origin(200, 120)    # 设置原点为中心
...
#        laser_canvas.add_waveform(160,120,math.sin,-150,150,1,50,1)
        ...
        while True:
            ...
            img.lens_corr(1.5)
#            motorx.velocity_control(1, 10, 255, False)
            
            Cxy = find_rectangles_565(img,MIN_AREA=10000,MAX_AREA=17000)
            if Cxy is not None:
...
#            rect = find_rectangles([320,240],img.to_rgb888())
#            print(rect)
#            if len(rect) > 3:
...
#                redLaser = recData[0]
            if redLaser is not None:
                ...
                if PID_Flag:
                    # 更新PID参数
                    # pidx.Kp = pid_controls[0].get_value()
                    # pidx.Ki = pid_controls[1].get_value()
                    # pidx.Kd = pid_controls[2].get_value()
                    
                    # pidy.Kp = pid_controls[3].get_value()
                    # pidy.Ki = pid_controls[4].get_value()
                    # pidy.Kd = pid_controls[5].get_value()
